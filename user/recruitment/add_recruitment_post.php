...
if ($postId > 0) {
    try {
        ...
        // Ensure the post belongs to the current DEO and is not approved/rejected
        if (!$post || $post['submitted_by_user_id'] !== $currentUserId || in_array($post['approval_status'], ['approved', 'rejected'])) {
...
if (isset($_POST['submit_recruitment_post'])) {
    ...
    if (empty($job_title) || empty($total_vacancies)) {
        ...
        // Note: contact_email is not in the HTML form, so removed validation for it here.
        // If you add it back to HTML, uncomment this.
        // $message = '<div class="alert alert-danger" role="alert">Invalid contact email format.</div>';
    } else {
        try {
            if ($postId > 0 && $post) { // Editing existing post
                // Update function needs to be adjusted to accept new fields
                if (updateRecruitmentPostHtml(
                    ...
                    $message = '<div class="alert alert-success" role="alert">Recruitment post updated successfully!</div>';
                ...
                // Add function needs to be adjusted to accept new fields
        ...
        <div class="card shadow-sm rounded-3">
            ...
            <div class="card-body">
                <form action="" method="POST" id="recruitmentPostForm">
                    ...
                    <fieldset class="bronline-fieldset border p-3 rounded-3 mb-3">
