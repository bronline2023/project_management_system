    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
                        ...
                        <div class="h-[180px] p-[1.5rem] flex flex-col justify-around items-center rounded-b-[20px] poster-footer">
                            ...
                            <div class="w-full text-center flex flex-col items-center">
                                ...
                                <div id="companyWebsiteUrlDisplay" class="text-[1rem] font-semibold dynamic-text-container">
                                    <i class="ph ph-buildings ph-icon"></i> Company Website: <img src="${data.companyLogoUrl}" alt="Company Logo" class="inline-block w-[24px] h-[24px] object-contain mx-1 align-middle" crossorigin="anonymous"> ${data.companyWebsiteUrl}
                                </div>
                            </div>
            ...
            els.downloadPosterBtn.addEventListener('click', function() {
                ...
                const originalButtonText = els.downloadPosterBtn.textContent;
                els.downloadPosterBtn.textContent = 'Generating Poster...';
                els.downloadPosterBtn.disabled = true;
                els.downloadPosterBtn.classList.add('opacity-70', 'cursor-not-allowed');

                // Explicitly re-run font fitting for critical elements right before capture
                fitTextToContainer(document.getElementById('organizationNameDisplay'), 2.3);
                fitTextToContainer(document.getElementById('recruitmentTitleDisplay'), 2.0);
                fitTextToContainer(document.getElementById('numberOfVacanciesDisplay'), 3.0);
                fitTextToContainer(document.getElementById('eligibilityCriteriaDisplay'), 1.2);
                ...
                // Get all images (including Phosphor Icons SVGs rendered as images by html2canvas) within the poster
                const images = posterElement.querySelectorAll('img, [class*="ph-"]'); // Selects both <img> and Phosphor Icon elements
                const assetLoadPromises = Array.from(images).map(asset => {
                    return new Promise(resolve => {
                        ...
                        } else if (asset.matches('[class*="ph-"]')) { // Check if it's a Phosphor Icon element
                            // For Phosphor Icons, they are usually loaded via CSS or JavaScript and rendered as SVG.
                            // html2canvas' foreignObjectRendering helps here. Assume they are ready with DOM.
                            resolve(); 
                        } else {
                            resolve(); // For any other unexpected element, just resolve
                        }
                    });
                });
                
                // Wait for all assets to load, then a slight delay for rendering, then capture
                Promise.all(assetLoadPromises).then(() => {
                    console.log('All images and SVGs processed. Proceeding with html2canvas capture after delay.');
                    setTimeout(() => {
                        console.log('Final rendering delay ended. Attempting html2canvas capture:', posterElement);
                        html2canvas(posterElement, {
                            ...
                            foreignObjectRendering: true, // Important for SVG/icon rendering
                            backgroundColor: '#ffffff', // Explicitly set background color
                            logging: true, // Enable logging for debugging
                        }).then(canvas => {
                            console.log('html2canvas capture successful. Canvas object:', canvas);
                            if (canvas.width === 0 || canvas.height === 0) {
                                console.error('Generated canvas has zero dimensions. This will result in a blank image. Check poster element visibility/dimensions.');
                            }
                            ...
                            link.download = 'recruitment_poster.png'; // PNG format
                            link.href = canvas.toDataURL('image/png'); // PNG format
                            // Log a snippet of the base64 URL to confirm content (for debugging)
                            console.log('Download link href (first 100 chars):', link.href.substring(0, 100) + '...');
                            ...
                            els.downloadPosterBtn.textContent = originalButtonText;
                            els.downloadPosterBtn.disabled = false;
                            els.downloadPosterBtn.classList.remove('opacity-70', 'cursor-not-allowed');

                        }).catch(error => {
                            console.error('html2canvas failed to render:', error);
                            // Restore original overflow styles in case of html2canvas error
                            dynamicTextContainers.forEach((container, index) => {
                                container.style.overflow = originalOverflows[index];
                            });
                            // Restore button state
                    ...
                    }, 2000); // **CRITICAL DELAY**: Gives browser time to render all assets (2.0 seconds)
                }).catch(error => {
                    console.error('Error in preloading assets before html2canvas:', error);
                    dynamicTextContainers.forEach((container, index) => {
                        container.style.overflow = originalOverflows[index];
                    });
                    els.downloadPosterBtn.textContent = originalButtonText;
