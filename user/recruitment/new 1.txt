...
<head>
    ...
    <style>
        ...
        .dynamic-text-container {
            /* Ensures text wraps and new lines are respected */
        ...
        #posterPreview {
            /* Fixed poster size for consistent output, 1080px height */
            width: 870px;
            height: 1080px;
            flex-shrink: 0;
            position: relative;
            /* Ensure high quality rendering for html2canvas */
        ...
        .poster-card {
            ...
            height: 100%; /* Ensures cards have a fixed height */
            display: flex;
            flex-direction: column;
        ...
        @media (max-width: 1023px) {
            ...
            #posterPreview { width: 425px; height: 540px; margin-top: 1rem; }
        }
    </style>
</head>
<body class="flex flex-col lg:flex-row min-h-screen">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            let customCompanyLogoUrl = 'https://placehold.co/120x120/ffffff/000000?text=Company'; // Default company logo
            ...
            // Handles file uploads and sets the image URL
            ...
            function updatePosterPreview() {
                ...
                const data = {
                    organizationName: els.organizationName.value,
                    recruitmentTitle: els.recruitmentTitle.value,
                    numberOfVacancies: els.numberOfVacancies.value,
                    applicationStartDate: getFormattedDate(els.applicationStartDate.value),
                    applicationDeadline: getFormattedDate(els.applicationDeadline.value),
                    eligibilityCriteria: els.eligibilityCriteria.value,
                    ageLimit: els.ageLimit.value,
                    feesDetails: els.feesDetails.value,
                    officialWebsiteUrl: els.officialWebsiteUrl.value,
                    companyWebsiteUrl: 'www.bronline.net',
                    officialLogoUrl: customOfficialLogoUrl,
                    companyLogoUrl: customCompanyLogoUrl,
                };
                
                // Poster HTML template with fixed dimensions and improved layout
                const posterHtml = `
                    <div id="posterPreview" class="relative poster-container ${selectedDesign}">
                        ...
                        <div class="h-[160px] p-[1.5rem] flex items-center justify-between bg-white bg-opacity-85 shadow-md rounded-t-[20px]">
                            <div class="w-[120px] h-[120px] bg-white p-[6px] rounded-[14px] shadow-lg flex items-center justify-center flex-shrink-0">
                                <img src="${data.officialLogoUrl}" alt="Official Logo" class="w-full h-full object-contain p-[4px] rounded-[10px]">
                            </div>
                            <div class="flex-grow text-center mx-[0.8rem] flex flex-col justify-center items-center h-full">
                                <div id="organizationNameDisplay" class="text-[2.3rem] font-extrabold text-red-600 dynamic-text-container">${data.organizationName || 'Organization Name'}</div>
                                <div id="recruitmentTitleDisplay" class="text-[2rem] font-bold text-indigo-700 mt-[0.5rem] dynamic-text-container">${data.recruitmentTitle || 'Recruitment Title'}</div>
                            </div>
                        </div>

                        <!-- Main grid content -->
                        <div class="p-[1.5rem] flex-grow grid grid-cols-2 gap-6">
                            <!-- Eligibility Card (Top Left) -->
                            <div class="poster-card flex flex-col justify-start h-[200px]">
                                ...
                                <div id="eligibilityCriteriaDisplay" class="text-[1.2rem] font-medium text-gray-700 dynamic-text-container flex-grow">${data.eligibilityCriteria || 'Eligibility details will be displayed here.'}</div>
                            </div>
                            <!-- Total Vacancies Card (Top Right) -->
                            <div class="poster-card flex flex-col justify-start h-[200px]">
                                <h4 class="poster-card-title pb-[0.3rem] w-full mb-[0.5rem]">
                                    <i class="ph ph-briefcase ph-icon"></i> Total Vacancies
                                </h4>
                                <div id="numberOfVacanciesDisplay" class="text-[3rem] font-extrabold text-green-600 dynamic-text-container">${data.numberOfVacancies || '0000'}</div>
                            </div>
                            <!-- Fees Card (Bottom Left) -->
                            <div class="poster-card flex flex-col justify-start h-[160px]">
                                <h4 class="poster-card-title pb-[0.3rem] w-full mb-[0.5rem]">
                                    <i class="ph ph-currency-circle-dollar ph-icon"></i> Fees
                                </h4>
                                <div id="feesDetailsDisplay" class="text-[1.2rem] font-medium text-gray-700 dynamic-text-container flex-grow">${data.feesDetails || 'Fee details will be displayed here.'}</div>
                            </div>
                             <!-- Age Limit Card (Bottom Right) -->
                            <div class="poster-card flex flex-col justify-start h-[160px]">
                                <h4 class="poster-card-title pb-[0.3rem] w-full mb-[0.5rem]">
                                    <i class="ph ph-user-circle ph-icon"></i> Age Limit
                                </h4>
                                <div id="ageLimitDisplay" class="text-[1.2rem] font-medium text-gray-700 dynamic-text-container flex-grow">${data.ageLimit || 'Age limit details will be displayed here.'}</div>
                            </div>
                        </div>
                        ...
                        <div class="h-[180px] p-[1.5rem] flex flex-col justify-around items-center rounded-b-[20px] poster-footer">
                            <div class="flex justify-around w-full mb-[1rem]">
                                ...
                                <div class="flex flex-col items-center">
                                    <p class="font-light text-sm">Start Date</p>
                                    <div id="startDateDisplay" class="mt-[0.2rem] text-[1.5rem] font-extrabold">${data.applicationStartDate || 'dd/mm/yyyy'}</div>
                                </div>
                                <!-- Deadline Date -->
                                <div class="flex flex-col items-center">
                                    <p class="font-light text-sm">Deadline</p>
                                    <div id="endDateDisplay" class="mt-[0.2rem] text-[1.5rem] font-extrabold text-red-400">${data.applicationDeadline || 'dd/mm/yyyy'}</div>
                                </div>
                            </div>
                            <!-- Website Links -->
                            <div class="w-full text-center flex flex-col items-center">
                                <div id="officialWebsiteUrlDisplay" class="text-[1rem] font-semibold dynamic-text-container mb-[0.2rem]">
                                    Official Website: ${data.officialWebsiteUrl || 'www.example.com'}
                                </div>
                                <div id="companyWebsiteUrlDisplay" class="text-[1rem] font-semibold dynamic-text-container">
                                    Company Website: ${data.companyWebsiteUrl || 'www.bronline.net'}
                                </div>
                            </div>
                            ...
                            <div class="w-[80px] h-[80px] bg-white p-[4px] rounded-[10px] shadow-md flex items-center justify-center mt-[1rem]">
                                <img src="${data.companyLogoUrl}" alt="Company Logo" class="w-full h-full object-contain p-[2px] rounded-[8px]">
                            </div>
                        </div>
                    </div>
                `;
