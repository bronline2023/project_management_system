...
if (isset($_POST['add_client'])) {
    ...
    // These fields are now optional, use null coalescing operator to default to empty string
    ...
    if (empty($name)) {
        ...
        try {
            // Updated INSERT statement to include new columns (contact_person, company)
            ...
            $_POST = array(); // Clears all POST data for the next load
            ...
            if ($e->getCode() == 23000) { // SQLSTATE for integrity constraint violation (e.g., duplicate email)
                ...
                error_log("Error adding client: " . $e->getMessage()); // Log detailed error
...
if (isset($_POST['edit_client'])) {
    ...
    // Basic validation: Client ID and Name are still required for editing
    if (empty($id) || empty($name)) {
        $message = '<div class="alert alert-danger" role="alert">Client Name and ID are required for editing.</div>';
    } elseif (!empty($email) && !filter_var($email, FILTER_VALIDATE_EMAIL)) {
        // Email is optional, but if provided, it must be a valid format
        $message = '<div class="alert alert-danger" role="alert">Invalid email format for editing.</div>';
    } else {
        try {
            // Updated UPDATE statement to include new columns (contact_person, company)
            ...
            if ($e->getCode() == 23000) { // SQLSTATE for integrity constraint violation (e.g., duplicate email)
                ...
                error_log("Error updating client: " . $e->getMessage()); // Log detailed error
...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    ...
    try {
        ...
        error_log("Error deleting client: " . $e->getMessage()); // Log detailed error
...
$clients = [];
try {
    // Updated SELECT statement to include new columns (contact_person, company)
    ...
    error_log("Error fetching clients for display: " . $e->getMessage()); // Log detailed error
                ...
                <form action="" method="POST">
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control rounded-pill" id="name" name="name" value="<?= htmlspecialchars($_POST['name'] ?? '') ?>" placeholder="Enter client name" required>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="contact_person" class="form-label">Contact Person</label>
                            <input type="text" class="form-control rounded-pill" id="contact_person" name="contact_person" value="<?= htmlspecialchars($_POST['contact_person'] ?? '') ?>" placeholder="Enter contact person's name">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="email" class="form-label">Email</label>
                            <input type="email" class="form-control rounded-pill" id="email" name="email" value="<?= htmlspecialchars($_POST['email'] ?? '') ?>" placeholder="Enter email address">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="phone" class="form-label">Phone</label>
                            <input type="text" class="form-control rounded-pill" id="phone" name="phone" value="<?= htmlspecialchars($_POST['phone'] ?? '') ?>" placeholder="Enter phone number">
                        </div>
                    </div>
                    <div class="row mb-3">
                        <div class="col-md-6">
                            <label for="company" class="form-label">Company</label>
                            <input type="text" class="form-control rounded-pill" id="company" name="company" value="<?= htmlspecialchars($_POST['company'] ?? '') ?>" placeholder="Enter company name">
                        </div>
                        <div class="col-md-6">
                            <label for="address" class="form-label">Address</label>
                            <textarea class="form-control rounded-3" id="address" name="address" rows="2" placeholder="Enter client address"><?= htmlspecialchars($_POST['address'] ?? '') ?></textarea>
                        </div>
                    </div>
            ...
            <form action="" method="POST">
                <div class="modal-body p-4">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control rounded-pill" id="edit-name" name="name" placeholder="Enter client name" required>
                    </div>
                    <div class="mb-3">
                        <label for="edit-contact-person" class="form-label">Contact Person</label>
                        <input type="text" class="form-control rounded-pill" id="edit-contact-person" name="contact_person" placeholder="Enter contact person's name">
                    </div>
                    <div class="mb-3">
                        <label for="edit-email" class="form-label">Email</label>
                        <input type="email" class="form-control rounded-pill" id="edit-email" name="email" placeholder="Enter email address">
                    </div>
                    <div class="mb-3">
                        <label for="edit-phone" class="form-label">Phone</label>
                        <input type="text" class="form-control rounded-pill" id="edit-phone" name="phone" placeholder="Enter phone number">
                    </div>
                    <div class="mb-3">
                        <label for="edit-company" class="form-label">Company</label>
                        <input type="text" class="form-control rounded-pill" id="edit-company" name="company" placeholder="Enter company name">
                    </div>
                    <div class="mb-3">
                        <label for="edit-address" class="form-label">Address</label>
                        <textarea class="form-control rounded-3" id="edit-address" name="address" rows="2" placeholder="Enter client address"></textarea>
                    </div>
                </div>
