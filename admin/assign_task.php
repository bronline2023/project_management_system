 ...
 *
 * This file handles the assignment of new work tasks by the administrator.
 * The admin can select a client, category, subcategory, assign it to a user,
 * set a work description, deadline, and define fee/maintenance fee details.
 *
...
if (isset($_POST['assign_task'])) {
    ...
    // Basic validation
    if (empty($client_id) || empty($assigned_to_user_id) || empty($category_id) || empty($subcategory_id) || empty($work_description) || empty($deadline) || !is_numeric($fee) || $fee < 0 || empty($fee_mode)) {
        $message = '<div class="alert alert-danger" role="alert">Please fill in all required fields (Client, User, Category, Subcategory, Description, Deadline, Fee, Fee Mode).</div>';
    } else {
        ...
        try {
            ...
            error_log("Error fetching subcategory fare: " . $e->getMessage());
        ...
        if (empty($message)) { // Proceed only if no prior errors
            // Ensure the fee matches the actual fare from DB, or adjust if business logic allows admin override
            // For now, we'll enforce the DB fare for security against client-side manipulation.
            if ($fee != $actual_fare) {
                 // Decide how to handle this:
                 // 1. Force $fee = $actual_fare;
                 $fee = $actual_fare;
                 // 2. Or, show an error and prevent submission if $fee != $actual_fare;
                 // $message = '<div class="alert alert-danger" role="alert">The entered fee does not match the subcategory\'s default fare.</div>';
            }
        }

        if (empty($message)) { // Proceed if no errors after fare check
            try {
                $stmt = $pdo->prepare("INSERT INTO work_assignments (client_id, assigned_to_user_id, category_id, subcategory_id, work_description, deadline, fee, fee_mode, maintenance_fee, maintenance_fee_mode, admin_notes, status, payment_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', 'pending')");
                $stmt->execute([
                    $client_id,
                    $assigned_to_user_id,
                    $category_id,
                ...
                $message = '<div class="alert alert-success" role="alert">Task assigned successfully!</div>';
            } catch (PDOException $e) {
                error_log("Error assigning task: " . $e->getMessage());
                $message = '<div class="alert alert-danger" role="alert">Error assigning task: ' . $e->getMessage() . '</div>';
            }
        }
    }
}
...
// Fetch Clients - Changed column to 'client_name' from 'name'
$clients = [];
try {
...
// Fetch Users (excluding admin for task assignment purposes, or include if desired)
$users = [];
try {
    // Fetch all users except the 'admin' role if admin users shouldn't be assigned tasks
    $stmt = $pdo->query("SELECT id, name, role FROM users WHERE role != 'admin' ORDER BY name ASC");
    $users = $stmt->fetchAll(PDO::FETCH_ASSOC);
    // If admin can also be assigned tasks, use: $stmt = $pdo->query("SELECT id, name, role FROM users ORDER BY name ASC");
} catch (PDOException $e) {
    error_log("Error fetching users: " . $e->getMessage());
    $message .= '<div class="alert alert-danger" role="alert">Error loading users.</div>';
}
    ...
    <div id="content" class="p-4 p-md-5 pt-5 w-100">
        ...
        <div class="card shadow-sm rounded-3 mb-4">
            <div class="card-header bg-primary text-white">
                <h5 class="mb-0"><i class="fas fa-clipboard-check me-2"></i>Task Details</h5>
            </div>
            <div class="card-body">
                <form action="" method="POST" id="assignTaskForm">
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" id="userSearchInput" class="form-control rounded-pill mb-2" placeholder="Search user...">
                            <select class="form-select rounded-pill" id="assigned_to_user_id" name="assigned_to_user_id" required>
                                <option value="">Select User</option>
                                <?php foreach ($users as $user): ?>
                                    <option value="<?= htmlspecialchars($user['id']) ?>"><?= htmlspecialchars($user['name']) ?> (<?= ucwords(htmlspecialchars(str_replace('_', ' ', $user['role']))) ?>)</option>
                                <?php endforeach; ?>
                            </select>
                            <small class="form-text text-muted">Type to search for a user.</small>
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control rounded-3" id="admin_notes" name="admin_notes" rows="2"></textarea>
                    </div>

                    <button type="submit" name="assign_task" class="btn btn-primary rounded-pill"><i class="fas fa-plus-circle me-2"></i>Assign Task</button>
                </form>
            </div>
        </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const userOptions = Array.from(userSelect.options); // Get all original options
        ...
        if (userSearchInput && userSelect) {
            userSearchInput.addEventListener('keyup', function() {
                ...
                // Clear current options, but keep the "Select User" option
                ...
                userOptions.forEach(option => {
                    // Skip the default "Select User" option during search
                    if (option.value === "") return;

                    const userName = option.textContent.toLowerCase();
                    if (userName.includes(searchTerm)) {
                        userSelect.appendChild(option); // Re-add matching options
                    }
                });
            });
        }
