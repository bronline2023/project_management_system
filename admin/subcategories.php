 ...
 *
 * This file handles the management of subcategories by the administrator.
 * It allows adding, editing, and deleting subcategories, and assigning
 * a specified fee (fare) value to each.
 *
...
// --- Fetch All Categories (for dropdown in add/edit forms) ---
$categories = [];
try {
    $stmt = $pdo->query("SELECT id, name FROM categories ORDER BY name ASC");
    $categories = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    error_log("Error fetching categories for subcategory forms: " . $e->getMessage());
    $message .= '<div class="alert alert-danger" role="alert">Error loading categories for subcategory forms.</div>';
}
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <button type="button" class="btn btn-sm btn-outline-danger rounded-pill"
                                                    onclick="showCustomConfirm('Delete Subcategory', 'Are you sure you want to delete subcategory: <?= htmlspecialchars($sub['subcategory_name']) ?>?', '<?= BASE_URL ?>?page=subcategories&action=delete&id=<?= htmlspecialchars($sub['id']) ?>')">
        ...
        <div class="modal-content rounded-4 shadow">
            ...
            <form action="" method="POST">
                ...
                <div class="modal-footer border-0 rounded-bottom-4">
                    ...
                    <button type="submit" name="edit_subcategory" class="btn btn-primary rounded-pill"><i class="fas fa-save me-2"></i>Save Changes</button>
                </div>
            </form>
        </div>
    </div>
