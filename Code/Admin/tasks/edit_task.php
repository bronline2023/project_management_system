 ...
 * [NEW] Added Payment Collected By field for balance calculation.
 */

// Block direct access to this file
if (!defined('ROOT_PATH')) {
    http_response_code(403);
    die('<strong>Error:</strong> This file cannot be opened directly. Please access it properly through the system.');
}
    ...
    <div class="card shadow-sm">
        ...
        <div class="card-body">
            <form action="?page=actions" method="post" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        <label for="payment_collected_by" class="form-label fw-bold text-warning">Payment Collected By</label>
                        <select name="payment_collected_by" id="payment_collected_by" class="form-select border-warning">
                            <option value="company" <?= ($task['payment_collected_by'] ?? 'company') == 'company' ? 'selected' : '' ?>>Company</option>
                            <option value="self" <?= ($task['payment_collected_by'] ?? '') == 'self' ? 'selected' : '' ?>>Freelancer / Self</option>
                        </select>
                        <small class="text-muted">Select who collected the payment.</small>
                    </div>
                </div>
                ...
                <p>The user has submitted this task for verification. You can either approve it as completed or return it for further work.</p>
                 <form action="?page=actions" method="post" enctype="multipart/form-data">
                    ...
                    <input type="hidden" name="task_id" value="<?= htmlspecialchars($task['id'] ?? '') ?>">
                     ...
                     <input type="hidden" name="payment_collected_by" value="<?= htmlspecialchars($task['payment_collected_by'] ?? 'company') ?>">
